                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
                    <div className="bg-white w-full max-w-lg rounded-[2.5rem] border border-slate-200 overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
                        <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                            <div>
                                <h2 className="text-2xl font-black text-slate-900">Confirmar Fiado</h2>
                        ...
                        <div className="p-8 space-y-6">
                            <div className="p-6 bg-indigo-50 rounded-3xl border border-indigo-100 text-center space-y-2">
                                <p className="text-xs font-black text-indigo-400 uppercase tracking-widest">Total a Crédito</p>
                                <p className="text-5xl font-black text-indigo-900">${total.toLocaleString('es-CL')}</p>
                            </div>

                            <p className="text-center text-slate-500 font-medium text-sm px-4">
                                Se registrará una deuda única por el total de la venta en la cuenta corriente del cliente.
                            </p>

                            <button
                                onClick={() => {
                                    // Generate single debt entry
                                    const singleDebt = [{
                                        monto: total,
                                        fecha_vencimiento: new Date(new Date().setDate(new Date().getDate() + 30)).toISOString().split('T')[0] // Default 30 days
                                    }]
                                    handleCheckout('cuotas', singleDebt)
                                }}
                                className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-black py-5 rounded-[1.5rem] shadow-xl shadow-indigo-600/20 transition-all active:scale-[0.98] uppercase tracking-widest text-sm flex items-center justify-center gap-3"
                            >
