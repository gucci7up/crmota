...
import { FileText, Search, Download, Filter, Eye, Tag, Loader2 } from 'lucide-react'
    ...
    const fetchSales = async () => {
        ...
        if (!error) {
            ...
            setStats({
                ...
                pending: data.filter(v => v.estado === 'pendiente').length
            })
        }
        setLoading(false)
    }
        ...
        <div className="space-y-10 animate-fade-in text-slate-900">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                ...
                <div className="flex gap-3">
                    <button className="flex items-center gap-2 px-6 py-4 bg-white border border-slate-200 text-slate-900 rounded-2xl font-bold hover:bg-slate-50 transition-all active:scale-95 shadow-sm">
                        <Download size={20} />
                        Reporte Mensual
                    </button>
                </div>
                                ...
                                filteredVentas.map(venta => (
                                    <tr key={venta.id} className="hover:bg-slate-50/50 transition-colors group">
                                        <td className="px-8 py-6">
                                            <span className="text-sm font-black text-slate-900 tracking-widest uppercase">VENTA-{venta.id.toString().padStart(4, '0')}</span>
                                        </td>
                                        <td className="px-8 py-6">
